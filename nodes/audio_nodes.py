...
class EagleAudioExtractor:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "extract_audio": ("BOOLEAN", {"default": True, "tooltip": "是否提取音频（取消则仅返回视频信息）"}),
            },
            "optional": {
        ...
        try:
            ...
            if extract_audio:
                ...
                if audio_codec == "copy（原始流）":
                    ...
                    args += ["-vn", "-c:a", "libvorbis", "-b:a", audio_bitrate if audio_bitrate != "lossless" else "192k"]
                elif audio_codec == "m4a":
                    args += ["-vn", "-c:a", "aac", "-b:a", audio_bitrate if audio_bitrate != "lossless" else "192k"]
                else:  # aac, mp3
                ...
                if result.returncode != 0:
                    return (self._empty_audio(), f"❌ 音频提取失败: {result.stderr.decode('utf-8', errors='replace')[-200:]}", 0.0, 44100)

                # 读取提取的音频
                waveform, sample_rate = self._read_audio(output_path)
                duration = len(waveform) / sample_rate if sample_rate > 0 else 0

                info = f"✅ 音频已提取: {output_path}\n⏱️ 时长: {duration:.2f}s\n🎵 采样率: {sample_rate}Hz\n📊 声道: {waveform.shape[0] if waveform.ndim > 1 else 1}"
    ...
    def _read_audio(self, audio_path: str) -> tuple:
        ...
        try:
            ...
            if data.ndim == 1:
                data = np.stack([data, data], axis=0)  # 单声道转立体声
            return data.T, sr  # 返回 (samples, channels)
        except ImportError:
            # soundfile 不可用，尝试用 scipy
            try:
...
class EagleAudioMixer:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "fade_in": ("FLOAT", {"default": 0.0, "min": 0, "max": 10.0, "step": 0.1, "tooltip": "淡入时长（秒）"}),
                "fade_out": ("FLOAT", {"default": 0.0, "min": 0, "max": 10.0, "tooltip": "淡出时长（秒）"}),
            },
            "optional": {
                "audio_a": ("AUDIO",),
                "audio_b": ("AUDIO",),
    ...
    def mix_audio(self, mix_mode: str, fade_in: float, fade_out: float,
        ...
        for audio in audio_list:
            if audio.shape[-1] != target_sr:
                import torch.nn.functional as F
                # 重采样
                orig_len = audio.shape[-1]
                new_len = int(orig_len * target_sr / 44100)  # 假设原采样率为 44100
                audio = F.interpolate(
        ...
        if mix_mode == "叠加混合":
            # 长度对齐到最长的
            max_len = max(a.shape[-1] for a in aligned)
            padded = []
            for a in aligned:
            ...
            # 直接拼接
            max_len = sum(a.shape[-1] for a in aligned)
            mixed = np.zeros((aligned[0].shape[0], max_len))
            offset = 0
            for a in aligned:
        ...
        info = f"✅ 混音完成\n🎚️ 混合模式: {mix_mode}\n⏱️ 时长: {mixed.shape[-1]/target_sr:.2f}s\n🎵 音量: A={volume_a}, B={volume_b}, C={volume_c}"
