...
一比一参将 ComfyUI-Danbooru-Gallery 实现。后端负责  - 速率限制5 req/min，保守取 30/min = 2s 间隔  - 统一 HTTP 请求入口(限流 + 429 退避)
  - aiohttp 路由：搜索图片详情/图片代理/收藏/认证/设置
...
def _wh_request(method: str, url: str, **kwargs):
    """统一 Wallhaven 请求入口：限流 + 默认 UA + 429 退避重试 + 网络异常捕获
    返回 (success: bool, response_or_error: Response|str)"""
    ...
    for attempt in range(2):
        ...
        try:
            ...
            logger.error(f"[Wallhaven] {err_msg} | URL={url}")
            return False, err_msg
        if resp is None:
            return False, "请求返回空响应"
...
async def verify_auth_route(request):
    try:
        ...
        try:
            ...
            if not ok:
                return web.json_response({"success": True, "valid": False, "network_error": True, "error": str(resp)})
...
async def search_route(request):
    try:
        ...
        ok, resp = await asyncio.to_thread(_wh_request, "GET", url, params=params, headers=headers, timeout=15)
        if not ok:
            return web.json_response({"success": False, "error": f"请求失败: {resp}"}, status=502)
        if resp.status_code != 200:
...
async def wallpaper_detail_route(request):
    try:
        ...
        ok, resp = await asyncio.to_thread(_wh_request, "GET", url, timeout=10)
        if not ok:
            return web.json_response({"success": False, "error": f"请求失败: {resp}"}, status=502)
        if resp.status_code != 200:
            return web.json_response(
...
async def collections_route(request):
    try:
        ...
        api_key = settings.get("api_key", "").strip()
        if not api_key:
            return web.json_response({"success": False, "error": "需API Key 才能访问收藏"})
...
async def collection_wallpapers_route(request):
    try:
        ...
        ok, resp = await asyncio.to_thread(_wh_request, "GET", url, params=params, timeout=15)
        if not ok:
            return web.json_response({"success": False, "error": f"请求失败: {resp}"}, status=502)
        if resp.status_code != 200:
            return web.json_response({"success": False, "error": f"HTTP {resp.status_code}"})
...
class WallhavenGalleryNode:
    """
    Wallhaven Gallery 将 ComfyUI 节点    内嵌 DOM widget(由 JS 注入)，选中后输IMAGE 张量列表和标签字符串列表    """
    _post_cache: dict = {}
    @classmethod
    def INPUT_TYPES(cls):
        return {
    ...
    def get_selected_data(self, bypass_image=None, selection_data="{}", **kwargs):
        """处理选中图片数据，下载原图并转换为张量，同时输出标签字符串。selection_data 将 ComfyUI widget 自动传入。若未选中且提供了 bypass_image，直接透传"""
        # ComfyUI 会将 widget 值作为关键字参数传入，直接使selection_data 即可
        ...
        try:
            ...
            logger.info(f"[Wallhaven] 解析{len(selections)} 个选中")
            if not selections:
                logger.warning("[Wallhaven] 无有效选中")
            ...
            for sel in selections:
                ...
                # 获取 tags - 搜索 API 不返tags，需要调用详API
                tags_str = ""
                if wallpaper_id:
                    try:
                        ok, detail_resp = _wh_request("GET", f"{BASE_URL}/w/{wallpaper_id}", timeout=10)
                        if ok and detail_resp.status_code == 200:
