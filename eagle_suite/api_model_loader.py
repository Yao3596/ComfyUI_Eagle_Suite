...
# ── 计费配置 ──────────────────────────────────────────────────
PRICING_MAP = {
    "ouyi-5-preview": {"input": 0.15, "output": 0.60},  # 每 1M tokens ($)
    "ouyi-5-preview-thinking": {"input": 0.15, "output": 0.60},
    ...
    "qwen-vl-max": {"input": 0.02, "output": 0.02}, # 示例值
...
class _BaseAPI:
    ...
    def _request(self, url: str, headers: dict, payload: dict, retries: int = None) -> tuple:
        last_err = ""
        ...
        for i in range(max_r + 1):
            try:
                if i > 0:
                    wait_time = self.backoff_factor * (2 ** (i - 1))
                    print(f"[EagleAPI] 重试中 ({i}/{max_r}), 等待 {wait_time:.1f}s...")
                ...
                last_err = f"请求超时 ({self.timeout}秒)"
                continue
        ...
        return False, None, f"请求失败 (已尝试 {max_r} 次重试): {last_err}", 0
...
class EagleAPIUnifiedNode(_BaseAPI):
    ...
    def process(self, api_config_key, api_config_url, api_config_model,
        ...
        try:
            ...
            # ── 计算成本 ───────────────────────────────────────────
            pricing = _get_pricing(mdl)
            cost = (prompt_tokens * pricing["input"] + completion_tokens * pricing["output"]) / 1000000
            ...
            status = (
                ...
                f"{prompt_tokens}→{completion_tokens} tokens{cost_str} | "
