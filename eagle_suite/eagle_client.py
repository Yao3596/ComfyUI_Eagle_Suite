...
class EagleClient:
    ...
    def resolve_folder_id(self, folder_id):
        """验证文件夹 ID；仅在易混字符形成唯一候选时自动纠正。"""
        value = str(folder_id or "").strip().upper()
        if not value:
            return None, False
        if self.get_folder_info_by_id(value):
            return value, False
        ...
        def walk(nodes):
            for node in nodes or []:
                candidate = str(node.get("id") or "").upper()
                if len(candidate) == len(value):
                    differences = [
                        (left, right) for left, right in zip(value, candidate) if left != right
                    ]
                    if len(differences) == 1 and frozenset(differences[0]) in ambiguous_pairs:
                        candidates.append(candidate)
                walk(node.get("children") or [])

        walk(self.get_folders())
        unique = list(dict.fromkeys(candidates))
        if len(unique) == 1:
            logger.warning(f"Eagle 文件夹 ID 已自动纠正: {value} -> {unique[0]}")
    ...
    def add_item_from_path(self, file_path, folder_id=None, name=None, tags=None, annotation=None, star=0, meta=None):
        ...
        path, folderId, name, tags 与 annotation。评分在导入成功后通过 item/update 写入。
        """
        if not file_path or not os.path.isfile(file_path):
            return {"status": "error", "message": f"导入文件不存在: {file_path}"}

        clean_tags = [str(tag).strip() for tag in (tags or []) if str(tag).strip()]
        clean_star = max(0, min(5, int(star or 0)))
        payload = {
        ...
        if annotation: payload["annotation"] = str(annotation)
                ...
                if res_data.get("status") == "success":
                    ...
                    if item_id:
                        ...
                        if not update_result.get("success"):
                            res_data["metadataWarning"] = update_result.get("message", "元数据更新失败")
                    return res_data
    ...
    def update_item(self, item_id, name=None, tags=None, annotation=None, star=None):
        """更新已有条目信息"""
        if not item_id:
            return {"success": False, "message": "缺少 item id"}
        data = {"id": item_id}
        if name is not None: data["name"] = str(name)
        if tags is not None: data["tags"] = [str(tag).strip() for tag in tags if str(tag).strip()]
        if annotation is not None: data["annotation"] = str(annotation)
        if star is not None: data["star"] = max(0, min(5, int(star)))
        try:
            response = requests.post(f"{self.base_url}/item/update", json=data, timeout=30)
            if response.status_code != 200:
                return {"success": False, "message": f"HTTP {response.status_code}: {response.text}"}
            result = response.json()
            if result.get("status") != "success":
                return {"success": False, "message": result.get("message") or str(result)}
