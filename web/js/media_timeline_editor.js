...
.emte-drop.hot{border-color:#42bff5;background:#14283b}.emte-main{display:grid;grid-template-columns:245px minmax(0,1fr);gap:8px;min-height:190px}
...
.emte-assets{height:190px;padding:7px;overflow:auto;display:flex;flex-direction:column;gap:5px}.emte-asset{display:grid;grid-template-columns:28px minmax(0,1fr) auto;gap:7px;align-items:center;padding:6px;border:1px solid #303a4d;border-radius:5px;background:#1b2130;cursor:grab}
...
.emte-clip{position:absolute;top:5px;bottom:5px;min-width:18px;border:1px solid #438bcc;border-radius:5px;background:linear-gradient(180deg,#244b70,#193752);overflow:hidden;cursor:pointer}.emte-clip.audio{border-color:#169968;background:linear-gradient(180deg,#0b6548,#074732)}.emte-clip.selected{box-shadow:0 0 0 2px #62c4ff inset}.emte-clip-title{display:block;padding:5px 6px 1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px;font-weight:700}.emte-clip-time{display:block;padding:0 6px;color:#b7c9db;font-size:9px}.emte-playhead{position:absolute;top:0;bottom:0;width:2px;background:#ffd21f;z-index:9;pointer-events:none}.emte-playhead:before{content:"";position:absolute;top:0;left:-5px;border-left:6px solid transparent;border-right:6px solid transparent;border-top:7px solid #ffd21f}
.emte-seek{position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:crosshair}.emte-settings{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:7px;padding:8px;flex-shrink:0}.emte-hint{color:#7f8ca1;font-size:10px}.emte-toggle{display:flex;align-items:center;gap:5px;padding-top:6px}.emte-file{display:none}
  ...
  setup(props) {
    ...
    const videoLayout = computed(() => {
      ...
      return project.value.video_clips.map((clip) => {
        ...
        const result = { clip, left: cursor / timelineDuration.value * 100, width: duration / timelineDuration.value * 100 };
        cursor += duration;
        return result;
      });
    });
    ...
    function addVideo(asset) {
      ...
      selected.value = { track: "video", id: project.value.video_clips.at(-1).id };
      renderedUrl.value = "";
      persist();
    }
    ...
    function selectClip(track, clip) {
      ...
      renderedUrl.value = "";
      if (player.value) currentTime.value = track === "video" ? clipGlobalStart(clip.id) : finite(clip.start);
    }
    ...
    function seek(event) {
      ...
      currentTime.value = Math.max(0, Math.min(timelineDuration.value, (event.clientX - rect.left) / rect.width * timelineDuration.value));
      if (player.value && Number.isFinite(player.value.duration)) player.value.currentTime = Math.min(player.value.duration || 0, currentTime.value);
    }
    function onPlayerTime() { if (player.value) currentTime.value = Math.min(timelineDuration.value, finite(player.value.currentTime)); }
    ...
    return {
      ...
      sourceUrl, ruler, videoLayout, audioLayout, assetName, formatTime, mediaUrl, uploadFiles, onDropFiles,
      ...
      duplicateSelected, splitAtPlayhead, removeAsset, seek, onPlayerTime, syncSize, persistSettings,
      ...
      <div class="emte-main">
        ...
        <div class="emte-panel">
          <div class="emte-player">
            <video v-if="previewAsset?.type==='video' || renderedUrl" ref="player" :src="sourceUrl" controls preload="metadata" @timeupdate="onPlayerTime"></video>
            <audio v-else-if="previewAsset?.type==='audio'" ref="player" :src="sourceUrl" controls @timeupdate="onPlayerTime"></audio>
      ...
      <div class="emte-timeline">
        ...
        <div class="emte-scroll">
          <div class="emte-canvas" :style="{width:canvasWidth+'px'}">
            <div class="emte-ruler"><div class="emte-label"></div><div class="emte-ruler-lane" @click="seek"><i v-for="mark in ruler" :key="mark.pct" class="emte-tick" :style="{left:mark.pct+'%'}"><span>{{ formatTime(mark.time) }}</span></i></div></div>
            <div class="emte-track"><div class="emte-label">🎞 V1</div><div class="emte-lane" @click="seek" @dragover.prevent @drop.prevent="dropTrack($event,'video')"><div v-for="(item,index) in videoLayout" :key="item.clip.id" class="emte-clip" :class="{selected:selected?.id===item.clip.id}" :style="{left:item.left+'%',width:item.width+'%'}" draggable="true" @dragstart.stop="startClipDrag('video',item.clip,index)" @dragover.prevent @drop.stop.prevent="reorderVideo(index)" @click.stop="selectClip('video',item.clip)"><span class="emte-clip-title">{{ assetName(item.clip.asset_id) }}</span><span class="emte-clip-time">{{ formatTime(item.clip.out-item.clip.in) }}</span></div><div class="emte-playhead" :style="{left:playheadLeft}"></div></div></div>
            <div v-for="trackIndex in [0,1]" :key="trackIndex" class="emte-track"><div class="emte-label audio">♪ A{{trackIndex+1}}</div><div class="emte-lane" @click="seek" @dragover.prevent @drop.prevent="dropTrack($event,trackIndex)"><div v-for="item in audioLayout(trackIndex)" :key="item.clip.id" class="emte-clip audio" :class="{selected:selected?.id===item.clip.id}" :style="{left:item.left+'%',width:item.width+'%'}" @click.stop="selectClip(trackIndex,item.clip)"><span class="emte-clip-title">{{ assetName(item.clip.asset_id) }}</span><span class="emte-clip-time">{{ formatTime(item.clip.out-item.clip.in) }}</span></div><div class="emte-playhead" :style="{left:playheadLeft}"></div></div></div>
          </div>
        </div>
      </div>
      <div class="emte-settings">
        <div class="emte-field"><label>输出内容</label><select v-model="outputMode" @change="persistSettings"><option value="video_audio">视频 + 音频</option><option value="video_audio_frames">视频 + 音频 + 帧</option><option value="frames">仅图像帧</option><option value="audio">仅音频</option></select></div>
