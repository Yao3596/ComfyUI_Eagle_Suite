...
var SelectionBar = {
  name: "SelectionBar",
  props: { selectedItems: Array, onRemove: Function, onClear: Function },
  setup: function(props) {
    return function() {
      // 预览条使用节点级全局选择对象，而不是当前文件夹的 items。
      // 这样切换文件夹/筛选条件后，已选图像仍然可见、可移除。
      var selected = props.selectedItems || [];
...
var EagleGallery = {
  ...
  setup: function(props) {
    ...
    function addSelectedItem(item) {
      ...
      var selection = selectionFromItem(item);
      var index = selectedItems.value.findIndex(function(sel) { return sel.id === selection.id; });
      if (index >= 0) selectedItems.value.splice(index, 1, selection);
      else selectedItems.value.push(selection);
      if (selectedIds.value.indexOf(selection.id) === -1) selectedIds.value.push(selection.id);
    }

    function removeSelectedId(id) {
      var idIndex = selectedIds.value.indexOf(id);
      if (idIndex >= 0) selectedIds.value.splice(idIndex, 1);
      var itemIndex = selectedItems.value.findIndex(function(sel) { return sel.id === id; });
      if (itemIndex >= 0) selectedItems.value.splice(itemIndex, 1);
    }
    ...
    function applySelections(selections) {
      ...
      (Array.isArray(selections) ? selections : []).forEach(function(sel) {
        if (!sel || sel.id == null || seen[String(sel.id)]) return;
        seen[String(sel.id)] = true;
        normalized.push(selectionFromItem(sel));
      });
      selectedItems.value = normalized;
      selectedIds.value = normalized.map(function(sel) { return sel.id; });
    }
    ...
    function syncSelection(customSelections) {
      ...
      var sels = Array.isArray(customSelections)
        ? customSelections.map(selectionFromItem)
        : selectedItems.value.map(selectionFromItem);
      if (Array.isArray(customSelections)) applySelections(sels);
    ...
    function selectAllForOutput() {
      ...
      items.value.forEach(function(item) {
        sels.push(selectionFromItem(item));
      });
      syncSelection(sels);
    }
    ...
    function onImg(item, index, e) {
      ...
      if (i >= 0) removeSelectedId(item.id);
    ...
    function selectByIndex(input) {
      ...
      list.forEach(function(idx) {
        if (idx >= 0 && idx < items.value.length) {
          addSelectedItem(items.value[idx]);
        }
      });
...
var CSS = [
  ...
  ".g-wrap::-webkit-scrollbar{width:12px}",
  ".g-wrap::-webkit-scrollbar-track{background:#171920;border-left:1px solid #292d36}",
  ".g-wrap::-webkit-scrollbar-thumb{background:#596273;border:3px solid #171920;border-radius:8px}",
  ".g-wrap::-webkit-scrollbar-thumb:hover{background:#748097}",
