...
| 🦅 **H3 · 参考条件路由** | `🦅 Eagle Suite/H3 导演台/参考条件` | 将导演台 `media_bundle` 按当前场景的 `<Picture #>` / `<Video #>` / `<Audio #>` 标签展开为官方 H3 独立参考槽位，并可注入上一镜上下文 Guide |
...
导演台的 `media_bundle` 只需一条线连接“参考条件路由”。参考图片、视频和音频必须保留为独立语义槽位；不要先拼成宫格，否则官方 H3 无法继续保证 `<Picture #>` 等标签与具体素材一一对应。需要检查原始槽位时，才使用“H3 媒体集成端口”展开查看。

“参考条件路由”的参考图尺寸支持 `match / custom / max`：`custom` 可用 `ref_short_edge` 在 640–2048px 间按 64px 调节短边上限。该设置只会缩小过大的参考图，不会放大低分辨率素材，避免无效插值和额外显存占用。
...
- ✨ **Context Loop Plan 桥接**：导演台新增 `plan_json` 输出；连接到第三方 `MiniMax H3 Context Loop Plan` 的 `plan_json_input`，再由该 Plan 的 `plan` 输出连接 Scene Prompt Editor / Plan Studio。第三方编辑器前端只识别它自己的 Plan 节点，不能直接把 Eagle 的 `plan` 线当作可编辑源。
